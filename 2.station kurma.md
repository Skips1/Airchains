...
> TRACKER CÜZDAN IMPORT (İlk station kurarken yazdığımız cüzdan ismini ve bize verilen kelimeleri girerek aynı cüzdanı import edeceğiz)


```console
go run cmd/main.go keys import --accountName CUZDANISMI --accountPath $HOME/.tracks/junction-accounts/keys --mnemonic 'bize verilen gizli kelimeler'
```

Account imported: airxxxxxxxxxxxxxxxxxxxxxxxxxx şeklinde size ait cüzdan adresini gösteren bir çıktı almanız lazım.
